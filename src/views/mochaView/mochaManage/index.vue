					...
					<div class="table-view">
						...
						>
							...
							<el-table-column label="保养周期">
								<template slot-scope="{ row }">
									<span>
										{{ getTitle(row.maintenanceCycle) }}
									</span>
								</template>
							</el-table-column>
							<el-table-column prop="maintenanceCycleStatus" label="保养状态">
							</el-table-column>
				...
				<div class="mocha-run">
					...
					<div class="table-view">
						...
						>
							...
							<el-table-column label="巡检周期">
								<template slot-scope="{ row }">
									<span> {{ getTitle(row.inspectCycle) }}</span>
								</template>
							</el-table-column>
							<el-table-column
								prop="inspectCycleStatus"
								label="巡检状态"
							></el-table-column>
							<el-table-column
      ...
      ></el-pagination>
		...
		<div class="model">
			...
			>
              ...
              ></el-autocomplete> -->
						...
						<el-form-item label="巡检周期">
							<el-select
								v-model.number="form.inspectCycle"
								placeholder="巡检周期"
							>
								<el-option
						...
						<template v-if="form.inspectCycle === 2 || form.inspectCycle === 3">
							<el-form-item label="巡检日期">
								<el-input
									v-model="form.inspectCycleDate"
									placeholder="每月填写格式：01(表示每月1号);每年填写格式：01-01(表示每年1月1号)"
								></el-input>
							</el-form-item>
							<el-form-item label="提前预警天数">
								<el-input v-model.number="form.warningDay"></el-input>
							</el-form-item>
						</template>
			...
			>
				<div>
					<el-form ref="form" :model="form" label-width="120px">
						...
						<template
							v-if="form.maintenanceCycle === 2 || form.maintenanceCycle === 3"
						>
							<el-form-item label="保养日期">
								<el-input
									v-model="form.maintenanceCycleDate"
									placeholder="每月填写格式：01(表示每月1号);每年填写格式：01-01(表示每年1月1号)"
								></el-input>
							</el-form-item>
							<el-form-item label="提前预警天数">
			...
			>
				...
				<span slot="footer" class="dialog-footer">
					<el-button type="primary" @click="fectVisibleInfo = false"
						>退 出</el-button
					>
				</span>
			</el-dialog>
...
export default {
	...
	methods: {
		...
		getTitle(type) {
			...
			return this.inspType.filter((v) => v.value === type)[0].label
		},
	},
}
</script>
